...
namespace TaskApp.App.Managers
{
    ...
    { 
        ...
        private XmlRootAttribute root = new XmlRootAttribute();
        ...
        {
            string xml = File.ReadAllText(@"C:\.Dev\.repos\KursASPNetMVC\Week5\Tasks.xml");
            root.ElementName = "Tasks";
            XmlSerializer xnlSerializer = new XmlSerializer(typeof(List<Task>), root);
        ...
        public void AddTask()
        {
            ...
            var task = new Task(_taskService.GetLastTaskId()+1,taskName,taskDescription,categoryId,categoryName,1,"New",priorityId,priorityName);
        ...
        private void AddTaskToXml(Task task)
        {
            root.ElementName = "Tasks";
            XmlSerializer xnlSerializer = new XmlSerializer(typeof(List<Task>), root);
            using StreamWriter sw = new StreamWriter(@"C:\.Dev\.repos\KursASPNetMVC\Week5\Tasks.xml");
            xnlSerializer.Serialize(sw, _taskService.Tasks);
        }
        private void RemoveTaskFromXml(int taskId)
        {
            string xml = File.ReadAllText(@"C:\.Dev\.repos\KursASPNetMVC\Week5\Tasks.xml");
            root.ElementName = "Tasks";
            XmlSerializer xnlSerializer = new XmlSerializer(typeof(List<Task>), root);
            StringReader stringReader = new StringReader(xml);
            var xnlTask = (List<Task>)xnlSerializer.Deserialize(stringReader);
            var taskFromXml = xnlTask.Where(t => t.Id == taskId).FirstOrDefault();
            _taskService.Tasks.Remove(taskFromXml);
            using StreamWriter sw = new StreamWriter(@"C:\.Dev\.repos\KursASPNetMVC\Week5\Tasks.xml");
            xnlSerializer.Serialize(sw, _taskService.Tasks);
        }
        private void UpdateXml(Task task)
        {
            string xml = File.ReadAllText(@"C:\.Dev\.repos\KursASPNetMVC\Week5\Tasks.xml");
            root.ElementName = "Tasks";
            XmlSerializer xnlSerializer = new XmlSerializer(typeof(List<Task>), root);
            StringReader stringReader = new StringReader(xml);
            var xnlTask = (List<Task>)xnlSerializer.Deserialize(stringReader);
            var taskFromXml = xnlTask.Where(t => t.Id == task.Id).FirstOrDefault();
            taskFromXml = task;
            using StreamWriter sw = new StreamWriter(@"C:\.Dev\.repos\KursASPNetMVC\Week5\Tasks.xml");
            xnlSerializer.Serialize(sw, _taskService.Tasks);
        }
